...
class ArtifactSpec extends AnyFlatSpec with should.Matchers {
  ...
  "Validate" should "throw on level too low" in {
    val artifact = new Artifact("Gambler", "Flower", -1, 5, "HP", 5, Map(atkFlat -> 12))
  ...
  "Validate" should "throw on level too high" in {
    List(1, 2).foreach(rarity => {
      ...
      val artifact = new Artifact("Gambler", "Flower", level, rarity, "HP", 5, Map(atkFlat -> 12))
      Artifact.validate(artifact).isFailure shouldBe true
    })
  ...
  "Validate" should "throw on rarity too low" in {
    ...
    val artifact = new Artifact("Gambler", "Flower", 0, rarity, "HP", 5, Map(atkFlat -> 12))
    Artifact.validate(artifact).isFailure shouldBe true
  }
  ...
  "Validate" should "throw on number of sub stats too low" in {
    val subStats = Map[StatNames, Double](atkFlat -> 12, elementalMastery -> 12,
    ...
    3 to 5 foreach (rarity => {
      val artifact = new Artifact("Gambler", "Flower", 0, rarity, hpFlat.toString, 430, subStats.take(rarity - 3))
      Artifact.validate(artifact).isFailure shouldBe true
    })
  }
  ...
  "Validate" should "do nothing on proper artifact" in {
    val artifact = new Artifact("Gambler", "Flower", 0, 3, "HP", 430, Map(atkFlat -> 12))
